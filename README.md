...
- **Loomy（讯飞）与其余七个都不同源**，有三点必须知情：
  - 它是唯一用**微信扫码**登录的：`auth.login.start` 返回的链接直接就是二维码页，页面内联渲染微信返回的二维码（`data:` URL，浏览器不再外连），随后靠 `<meta http-equiv="refresh">` 自刷新——每次加载只做**一次**长轮询（408 待扫码 / 404 已扫码继续轮询 / 405 已确认并携带授权码 / 403 取消 / 402 过期），405 之后自动走 `bind/auth`，`bind===1` 直接 `bind/skip`，否则在同一页用数字键盘绑定手机号。微信回调页只是白名单占位、本身 404，授权码完全来自长轮询。**短信验证码登录仍然保留**，作为同页的备用路径：它需要一个 11 位手机号，号码既可以用页面上的数字链接输入（本页没有表单，也没有脚本），也可以配置默认值或用 `?phone=` 覆盖。
...
- **多账号有显式入口**：状态页的「新建账号」链接到本插件自己的登录页（`login?add=1`），登录页用一行字说明这是新增账号；凭据文件名由账号自身身份推导（各 provider 的 `defaultAuthFileName`），所以第二个账号落在独立文件里，已有账号不受影响。这是纯导航改动——没有新增路由。
