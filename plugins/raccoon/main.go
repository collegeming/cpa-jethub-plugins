// Command raccoon builds the CPA c-shared plugin library for Raccoon Work (商汤小浣熊).
//
...
// `go build -buildmode=c-shared ./plugins/raccoon` (see scripts/build.sh) emit the
// shared object directly from this directory. The dynamic library file name must
// start with the plugin id (`raccoon.so` or `raccoon-v<version>.so`); any other
// spelling makes the host derive the wrong id, and the resource routes are
// mounted under `/v0/resource/plugins/<id>/`.
package main
