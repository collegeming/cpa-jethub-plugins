...
// fakeJWT mints an unsigned JWT whose payload carries `exp`, which is all this
// plugin ever reads from one (`decodeJwtExpMs` does not verify signatures).
func fakeJWT(t *testing.T, exp time.Time) string {
	t.Helper()
	header := base64.RawURLEncoding.EncodeToString([]byte(`{"alg":"HS256","typ":"JWT"}`))
	payload := base64.RawURLEncoding.EncodeToString([]byte(
		fmt.Sprintf(`{"exp":%d,"nbf":%d,"sub":"raccoon"}`, exp.Unix(), exp.Add(-3*time.Hour).Unix())))
	return header + "." + payload + ".signature"
}

// sampleCredential builds a credential whose JWT expires at exp.
func sampleCredentialAt(t *testing.T, exp time.Time) *Credential {
	t.Helper()
	credential := buildCredential(fakeJWT(t, exp), "refresh-token-1", "personal")
	credential.UserID = "user-alice"
	credential.Nickname = "RaccoonAva"
	credential.Phone = "13800138000"
	return credential
}

// sampleCredential is the canonical live credential: three hours left.
func sampleCredential(t *testing.T) *Credential {
	t.Helper()
	return sampleCredentialAt(t, time.Now().Add(3*time.Hour))
}
...
func storedCredential(t *testing.T, fake *fakeHost, authIndex, name string, credential *Credential) pluginapi.HostAuthFileEntry {
	...
	return fake.files[len(fake.files)-1]
}
...
// base64URLEncode encodes a JWT segment without padding, the way issuers write
// it.
