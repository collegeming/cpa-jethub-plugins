...
// renews it itself, before it is used. The executor's existing post-401 retry
// stays as the last line of defence for a credential the server rejects early.
...
func newCredentialRefresher() *authrefresh.Refresher {
	return authrefresh.New(authrefresh.Policy{
		...
		// The provider's own `auth.refresh` handler is the renewal: it and the
		// executor's retry already share renewCredential, so renewal has exactly
		// one implementation.
		Refresh: authrefresh.Handler(handleAuthRefresh),
	})
}

// credentialExpiry reports the expiry the credential actually carries, from
// `expires_at` or the access token's `exp` claim. ok=false means "unknown",
// which leaves the credential alone — the server stays the only authority.
