...
// successful login looks like "尚未添加账号" on the plugin's own page.
//
...
func TestEncodeRepairsMissingProviderType(t *testing.T) {
	legacy := []byte(`{"access_key_id":"HSTAUN898E1B7XD3G4OV","secret_access_key":"secret","security_token":"token","expires_at":"2999-01-01T00:00:00Z"}`)
...
func providerTypeCredential() *Credential {
	return &Credential{
		AccessKeyID:     "HSTAUN898E1B7XD3G4OV",
...
func TestProviderTypeSurvivesTimestampCoercion(t *testing.T) {
	raw := []byte(`{"type":"` + ProviderKey + `","access_key_id":"HSTAUN898E1B7XD3G4OV","secret_access_key":"secret","security_token":"token","expires_at":1799999999}`)
	...
	if parsed.ExpiresAt != "1799999999" {
