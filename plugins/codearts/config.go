...
type Config struct {
	...
	// CodeArts portal derives host and path itself and only accepts a port
	// number, so the default oauth flow cannot be redirected anywhere else.
	// Zero keeps the historical behaviour: an ephemeral port >= 10000.
	//
