...
// failed comes back inside the result (Result.Err) instead, because it does not
// stop the caller from showing the credential's own fields, and — while the
// credential is still inside its validity — does not stop the upstream read
// either. Result.Expired says whether the credential is past its expiry anyway.
...
func renderCheckinOutcome(h *abiboot.Host, entry pluginapi.HostAuthFileEntry) template.HTML {
	...
	// one that could not be renewed would only produce an upstream rejection, so
	// the renewal failure is stated instead.
...
func checkinJSON(h *abiboot.Host, request pluginapi.ManagementRequest) pluginapi.ManagementResponse {
	...
	if freshness.Expired && freshness.Err != nil {
		return jsonManagementResponse(http.StatusBadGateway, map[string]any{
			"error": "凭据已过期且自动续期失败：" + freshness.Err.Error(),
		})
	}
	outcome, errClaim := claimDaily(h, credential)
	if errClaim != nil {
