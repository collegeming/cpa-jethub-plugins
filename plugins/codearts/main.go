...
func cliproxy_plugin_init(host *C.cliproxy_host_api, plugin *C.cliproxy_plugin_api) C.int {
	...
	abiboot.SetHostCaller(hostCall)
...
// hostCall is the cgo trampoline used for every host callback.
func hostCall(method string, request []byte) ([]byte, error) {
	...
	if len(request) > 0 {
		...
		if cPayload == nil {
			return nil, abiboot.Errorf("host_alloc", "allocate host request for %s", method)
		}
