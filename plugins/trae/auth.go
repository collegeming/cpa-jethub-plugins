...
// nickname and then the uid; a credential that arrived without either (an
// imported auth file) would otherwise be renamed on the next save, which is how
// one account ends up with several auth files.
