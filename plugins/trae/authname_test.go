...
//
// A credential that arrived without a nickname and a uid — an auth file imported
// from Jet-Hub, say — derives its name from the shared constant "account"; if a
// refresh used that name it would collect every such account into one file.
// Only the name the host hands over keeps each credential where it is.
func TestRefreshKeepsTheCredentialsOwnAuthFileName(t *testing.T) {
	existing := "trae-uid-1.json"
...
func TestRefreshFallsBackToThePathAttribute(t *testing.T) {
	refreshed := &Credential{Type: ProviderKey, AccessToken: "tok", UID: "uid-1"}
	got := authNameForHost("", "/root/.cli-proxy-api/trae-uid-1.json", "", refreshed)
	if got != "trae-uid-1.json" {
...
func TestNewLoginStillDerivesDistinctNames(t *testing.T) {
	first := authNameForHost("", "", "", &Credential{Type: ProviderKey, UID: "uid-1", Nickname: "张三"})
	second := authNameForHost("", "", "", &Credential{Type: ProviderKey, UID: "uid-2", Nickname: "李四"})
	if first == second {
