...
//
// The CodeBuddy family appears as four separate CPA plugin ids: one source tree
// is built once per product (`scripts/build.sh` VARIANTS) because a CPA plugin
// registers exactly one provider key. A host that installs only one variant will
// simply report the other three as unreachable.
func targetCatalogue() []target {
	return []target{
		{
			ID: "codearts", Label: "CodeArts Agent", Support: supportStatusHTML,
			CheckinPath: "/status",
			CheckinQuery: url.Values{
		...
		{
			ID: "codebuddy", Label: "CodeBuddy", Support: supportJSON,
			CheckinPath:  "/checkin",
			CheckinQuery: url.Values{"action": {"checkin"}},
			Note:         "只有启用签到接口的产品才注册 /checkin 资源路由，其它产品会如实报告不支持",
		},
		{
			ID: "codebuddy-intl", Label: "CodeBuddy（国际版）", Support: supportJSON,
			CheckinPath:  "/checkin",
			CheckinQuery: url.Values{"action": {"checkin"}},
			Note:         "同一份 codebuddy 源码按产品构建的独立插件 id；未安装时报告未启用",
		},
		{
			ID: "workbuddy-cn", Label: "WorkBuddy（国内版）", Support: supportJSON,
			CheckinPath:  "/checkin",
			CheckinQuery: url.Values{"action": {"checkin"}},
			Note:         "同一份 codebuddy 源码按产品构建的独立插件 id；未安装时报告未启用",
		},
		{
			ID: "workbuddy", Label: "WorkBuddy（国际版）", Support: supportJSON,
			CheckinPath:  "/checkin",
			CheckinQuery: url.Values{"action": {"checkin"}},
			Note:         "同一份 codebuddy 源码按产品构建的独立插件 id；未安装时报告未启用",
		},
		{
			ID: "qoder", Label: "Qoder", Support: supportJSON,
			CheckinPath: "/checkin",
		...
		{
			ID: "trae", Label: "TRAE", Support: supportJSON,
			CheckinPath: "/checkin",
		...
		{
			ID: "lobsterai", Label: "LobsterAI", Support: supportJSON,
			CheckinPath: "/checkin",
		...
		{
			ID: "loomy", Label: "Loomy（讯飞）", Support: supportJSON,
			CheckinPath: "/checkin",
		...
		{
			ID: "cline", Label: "Cline", Support: supportNone,
