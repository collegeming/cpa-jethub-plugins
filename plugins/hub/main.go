// Command hub builds the CPA c-shared plugin library that drives the one-click
// check-in across every provider plugin.
//
...
// `go build -buildmode=c-shared ./plugins/hub` (see scripts/build.sh) emit the
// shared object directly from this directory.
//
// The artifact MUST be named `hub.so` or `hub-v<version>.so`: CPA derives the
// plugin id — and therefore the `/v0/resource/plugins/<id>/...` prefix this
// plugin's own pages live under — from the file name stem.
package main
