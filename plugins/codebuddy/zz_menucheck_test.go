...
func TestMenuCount(t *testing.T) {
	...
	//
	// /checkin only exists for the products that have a check-in endpoint, so the
	// expectation is derived the same way the registration derives it.
	product, _ := productByConfigValue(settings().Product)
	want := []string{"/status", "/login"}
	if supportsCheckin(product) {
		want = append(want, "/checkin")
	}
