...
// A token that expires inside such a window is never renewed by the host, so
// the plugin renews it itself, before it is used. One wiring therefore covers
// every build variant of this source tree (codebuddy / codebuddy-intl /
// workbuddy-cn / workbuddy): only the provider key differs.
...
// credentialExpiry reports the expiry the credential actually carries. An
// unparseable or missing expiry reports ok=false — "unknown expiry is not
// expired" — and leaves the credential alone, exactly as the reference treats
// it.
