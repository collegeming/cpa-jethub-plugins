...
// renews it itself, before it is used. The executor's own expired-credential
// renewal and its post-401 retry both stay as the last line of defence.
...
func newCredentialRefresher() *authrefresh.Refresher {
	return authrefresh.New(authrefresh.Policy{
		...
		// The provider's own `auth.refresh` handler is the renewal: it already
		// shares refreshCredential with the executor, so renewal has exactly one
		// implementation.
		Refresh: authrefresh.Handler(handleAuthRefresh),
	})
}

// credentialExpiry reports the expiry the credential actually carries. A zero
// expiry means "no expiry information" — not "expired in 1970" — and leaves the
// credential alone, exactly as the reference treats it.
