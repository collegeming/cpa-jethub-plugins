// Command loomy builds the CPA c-shared plugin library for Loomy (讯飞办公助手).
//
...
// `go build -buildmode=c-shared ./plugins/loomy` (see scripts/build.sh) emit the
// shared object directly from this directory. The dynamic library file name must
// start with the plugin id (`loomy.so` or `loomy-v<version>.so`); any other
