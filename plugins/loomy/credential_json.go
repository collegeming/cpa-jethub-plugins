...
// UnmarshalJSON decodes a Loomy credential as the host may hand it back.
//
// `expires_at` is a STRING millisecond timestamp in the credential contract
// (`loomy.ts:63-69`), but the host re-serialises credentials it stores and writes
// a numeric-looking string back as a JSON number. Decoding that into the plain
// `string` field fails, and the failure is easy to miss because the provider then
// simply publishes no models and every executor call reports a broken
// credential. CoerceToStrings rewrites only the named field and leaves every
// other spelling untouched.
...
// credential (`loomy.ts:177-182`, trap #17), and losing the whole credential over
// one unusable timestamp would be far worse than losing the timestamp.
