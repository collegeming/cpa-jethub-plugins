...
func TestEncodeRepairsMissingProviderType(t *testing.T) {
	legacy := []byte(`{"security_oauth_token":"qoder-oauth-token","access_token":"qoder-oauth-token","machine_id":"0123456789abcdef0123456789abcdef","uid":"user-1"}`)
...
func providerTypeCredential() *Credential {
	return &Credential{
		SecurityOAuthToken: "qoder-oauth-token",
		AccessToken:        "qoder-oauth-token",
...
func TestProviderTypeSurvivesTimestampCoercion(t *testing.T) {
	raw := []byte(`{"type":"` + ProviderKey + `","security_oauth_token":"qoder-oauth-token","access_token":"qoder-oauth-token","machine_id":"0123456789abcdef0123456789abcdef","uid":"user-1","expire_time":"1799999999000"}`)
