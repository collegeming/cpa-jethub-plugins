...
//
// The refreshed credential carries a new bearer token, and a credential without
// a nickname or a uid derives its name from the first eight characters of that
// token — i.e. the derived name points at a different file on every refresh.
// Only the name the host hands over keeps the credential in place.
func TestRefreshKeepsTheCredentialsOwnAuthFileName(t *testing.T) {
	existing := "qoder-cn-abcdef12.json"
	refreshed := &Credential{Type: ProviderKey, AccessToken: "new-bearer-token", RefreshToken: "r", ExpireTime: 1799999999000}
	if derived := defaultAuthFileName(&Credential{AccessToken: "new-bearer-token"}); derived == existing {
...
func TestRefreshFallsBackToThePathAttribute(t *testing.T) {
	refreshed := &Credential{Type: ProviderKey, AccessToken: "tok", UID: "u-1"}
	got := authNameForHost("", "/root/.cli-proxy-api/qoder-cn-u-1.json", "", refreshed)
	if got != "qoder-cn-u-1.json" {
...
func TestNewLoginStillDerivesDistinctNames(t *testing.T) {
	first := authNameForHost("", "", "", &Credential{Type: ProviderKey, UID: "u-1", Nickname: "alice"})
	second := authNameForHost("", "", "", &Credential{Type: ProviderKey, UID: "u-2", Nickname: "bob"})
	if first == second {
