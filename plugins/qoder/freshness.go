...
// the executor, so a page load and an inference request arriving together cost
// the vendor ONE renewal.
//
...
func newCredentialRefresher() *authrefresh.Refresher {
	return authrefresh.New(authrefresh.Policy{
		...
		// second implementation of the device-token exchange to drift.
		Refresh: authrefresh.Handler(handleAuthRefresh),
	})
}

// credentialExpiry reports the expiry the credential actually carries. A
// credential without one (ExpireTime <= 0) is left alone: the server stays the
// only authority on it, exactly as the reference treats it.
